...
- [ ] **AC1** — A barra de navegação exibe cinco abas: `Board`, `Chain`, `Metrics`, `ADRs`, `REQs`.
      Alternar entre elas usa o mecanismo `switchView()` já existente, sem recarregar a página.
- [ ] **AC2** — A aba `ADRs` lista **exatamente 12** itens neste repositório com o filtro em
      "Todos"; a aba `REQs` lista **exatamente 58**. A contagem visível confere com
...
- [ ] **AC3** — Cada linha exibe identificador, título e status. O status é apresentado com
      indicador visual (cor/chip) legível em tema claro e escuro.
- [ ] **AC4** — O filtro de status é **derivado dinamicamente** dos valores presentes na
...
- [ ] **AC5** — Existe busca textual que filtra por título e por identificador,
      case-insensitive e acento-insensitive. Busca vazia restaura a lista completa.
- [ ] **AC6** — Clicar numa linha (ou pressionar Enter/Space com foco nela) abre o **drawer
...
- [ ] **AC7** — Acessibilidade: as abas novas seguem o padrão das existentes
...
- [ ] **AC8** — Estado vazio tratado: se um tipo não tiver itens (ou o filtro não casar nada),
...
- [ ] **AC9** — **Nenhum arquivo novo** é adicionado a `internal/serve/static/`. As mudanças
      ocorrem apenas em `app.js`, `index.html` e `style.css`.
- [ ] **AC10** — `npm/src/serve/static/` e `pypi/trackfw/serve/static/` são espelhos
...
- [ ] **AC11** — `make build`, `make test`, `make lint` e `make parity` passam sem erro.
- [ ] **AC12** — Verificação visual manual nos três CLIs: `trackfw serve` (Go, após
