...
## Absorção do issue #310 (artefatos de `init` divergentes) — 2026-09-12

Decisão do KG: *"mensagens entram no Python, o Go para mim é expressão da verdade pois usamos ele
aqui em nosso ambiente."*

### A divergência é maior do que o issue relata

O #310 cita o shebang. Medido por execução dos três `init` na 7.6.0, **o script inteiro difere**:

| | Go / Node | Python |
|---|---|---|
| shebang | `#!/usr/bin/env sh` | `#!/usr/bin/env bash` |
| `set` | `-e` | `-euo pipefail` |
| comentário de cabeçalho | `# trackfw governance gate — generated by trackfw init` | ausente |
| mensagem de progresso | `→ trackfw: validating governance...` | **ausente** |
| mensagem de sucesso | `✓ all checks passed.` | **ausente** |
| linhas | 9 | 3 |

E o `init` do Python cria **um arquivo a mais**: `docs/adr/ADR-001-inicio-do-projeto.md` (20 arquivos
contra 19 de Go e Node).

Nenhum dos dois scripts usa construção bash-only — verificado. O `bash` do Python é desnecessário.

### Por que é a mesma causa desta REQ

**Nenhum gate compara o conteúdo dos artefatos gerados pelos três `init`.** Medido:
`check-artifact-parity.sh` compara REQ e roadmap (títulos, slugs), não os scripts;
`check-doctor-parity.sh` apenas **escreve** `trackfw-validate.sh` como fixture para o `doctor`
inspecionar; nenhum outro gate o menciona.

É a terceira instância do mecanismo desta REQ — **cobertura de paridade declarada por lista literal,
então superfície que não entra na lista nunca é verificada**:

```
AC4        conjunto de REGRAS do validator      não enumerado
AC9 (#298) conjunto de SUBCOMANDOS              não enumerado
AC14 (#310) conjunto de ARTEFATOS de init       não enumerado
```

### Decisão de convergência — tomada, não inferida

🔴 **O Go é a referência.** Razão do KG: é o runtime usado no ambiente real do projeto. Portanto o
Python **ganha** o que lhe falta, e nada é removido de Go/Node:

- Python passa a emitir `→ trackfw: validating governance...` e `✓ all checks passed.`
- Python passa a emitir o comentário de cabeçalho
- Python converge para `#!/usr/bin/env sh` e `set -e`
- Python **para de criar** `docs/adr/ADR-001-inicio-do-projeto.md`

> ⚠️ Consequência assumida: o Python **perde** `set -u` e `pipefail`. Se alguém quiser a variante
> estrita, ela vira mudança nos **três** runtimes, com `set -eu` (POSIX) e `pipefail` descartado por
> não ser POSIX — não é escopo desta REQ, e fica registrado aqui para não se perder.

### Critérios acrescentados

- [ ] **AC14** — gate que compara **byte a byte** os artefatos gerados por `trackfw init` entre os
      três runtimes. Falsificação: alterar um byte de um artefato num runtime ⇒ reprova nomeando
      arquivo e runtime. Contra-braço: árvore intacta ⇒ passa.
- [ ] **AC15** — 🔴 o gate compara o **conjunto de arquivos criados**, não só o conteúdo dos que
      existem nos três. Foi o conjunto que denunciou o `ADR-001` extra — um gate que só comparasse
      arquivos comuns não o veria.
- [ ] **AC16** — Python converge para o Go: mensagens, comentário de cabeçalho, `#!/usr/bin/env sh`,
      `set -e`, e sem criar o `ADR-001`. Provado pelo AC14 passando.
