...
roadmap: "docs/roadmaps/wip/ROADMAP-2026-08-01-corrigir-403-em-links-markdown-relativos-dentro-do-drawer.md"
---
...
- [ ] **AC1** — O href relativo é resolvido contra `dirname` do documento aberto (`_drawerPath`),
      normalizando `.` e `..`, **antes** de chamar `openDrawer`.
- [ ] **AC2** — As três formas relativas funcionam: `./X.md`, `X.md` (nu) e `../dir/X.md`.
      Verificado em navegador real abrindo o link e confirmando que o drawer troca de conteúdo.
- [ ] **AC3** — Caso concreto do repositório: em `docs/req/REQ-2026-06-13-validator-improvements.md`,
      o link `../roadmaps/done/v2.3-validator-improvements-2026-06-13.md` abre o roadmap.
- [ ] **AC4** — **Navegação encadeada:** A → B → C resolve cada salto contra o documento **então**
      aberto, não contra o primeiro. Verificado com três documentos em diretórios diferentes.
- [ ] **AC5** — Link que resolva para **fora** dos diretórios permitidos exibe mensagem
...
- [ ] **AC6** — Os 3 links `../vault/notes/*.md` caem no AC5 — **continuam não abrindo**, agora
      com mensagem clara. O whitelist **não** é alterado.
- [ ] **AC7** — Links externos (`http://`, `https://`) seguem **não** interceptados, com o
      comportamento atual de navegação preservado.
- [ ] **AC8** — Âncoras (`#secao`) e links não-`.md` seguem não interceptados.
- [ ] **AC9** — O primeiro documento aberto (via card do Board, nó do grafo Chain ou linha das
      listas ADRs/REQs) continua funcionando — a resolução não pode quebrar o caminho de entrada,
      onde o path já vem completo.
- [ ] **AC10** — npm e pypi byte-a-byte idênticos ao canônico;
      `scripts/check-static-assets.sh` imprime `Static assets are synchronized`.
- [ ] **AC11** — `make build`, `make test`, `make lint`, `make parity` e `make quality` verdes.
