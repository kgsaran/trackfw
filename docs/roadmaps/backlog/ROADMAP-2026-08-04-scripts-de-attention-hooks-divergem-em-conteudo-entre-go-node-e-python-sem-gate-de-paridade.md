...
- [ ] Os dois scripts (signal e cleanup) byte-idênticos entre Go, Node.js e Python
- [ ] Gate de paridade novo cobrindo esses dois scripts, integrado a `make quality`
- [ ] `docs/cli-parity.md` documenta o gate
- [ ] `trackfw init`/`update`/`discover --init` sem regressão em nenhum runtime
...
**Acceptance criteria:**
- [ ] `diff` vazio entre os três runtimes para os dois scripts (confirmado empiricamente, não só por
      leitura de código)
- [ ] `go build ./...`, `go test ./internal/...` verdes
- [ ] `npm test` verde
- [ ] `python3 -m pytest` verde (a partir de `pypi/`)
...
**Acceptance criteria:**
- [ ] Gate falha de propósito quando um dos três textos é alterado manualmente (prova de
      falsificabilidade — P4, seguindo `docs/gate-design-principles.md`)
- [ ] Gate passa no estado corrigido da Wave 1
- [ ] `make quality` verde
- [ ] `trackfw validate` sem violações
