...
**Ações:**
1. **Enumeração pela forma**, não pelo nome: quais regras do validador decidem sem consultar o estado
   do artefato? 🔴 **Não parar nos dois sítios já medidos** (`traceid_orphan_req`,
...
3. Decidir, com medição, **quais estados isentam**: `backlog` é certo; `abandoned` provavelmente;
   ...
   não exigir roadmap.
4. **Threat model:** quem esvazia esta Wave 0 sem quebrar regra escrita? Em particular: uma isenção
