...
py_raw=$(PYTHONPATH="$ROOT/pypi" python -c 'import sys;print(sys.stdin.isatty())' </dev/null)
py_fix=$(PYTHONPATH="$ROOT/pypi" python -c 'from trackfw.tty import stdin_is_interactive;print(stdin_is_interactive())' </dev/null)
