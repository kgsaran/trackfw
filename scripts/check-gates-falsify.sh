...
Prosa sem anotação nenhuma — modo relatório, não reprova.
EOF
}

# --- 77a — baseline: as 4 formas válidas + 1 seção sem anotação, exit 0 ----
...
for expected in \
  "total de seções reais (##/###/####), fora de fences: 5  (## 2 · ### 2 · #### 1)" \
...
echo "OK   [falsify/parity-contract-coverage/baseline]: 3 níveis de título + 4 estados válidos + 1 seção sem anotação, contagens corretas"

# --- 77b — gate= sem caminho nomeado (vazio) -------------------------------
...
assert_fails_with "parity-contract-coverage/gate-empty" \
  "gate= sem caminho nomeado (vazio)" \
