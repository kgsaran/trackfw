...
#               deixa de detectar a forma relativa antiga de Claude (check
#               requiresVarOrShellPrefix desativado por `false &&`) →
#               o novo caso cg-claude-relativo reprova.
#
...
# relativa antiga quebrasse.
#
# Seam: internal/validator/validator_credential_guard.go linha
# `if hf.requiresVarOrShellPrefix && isRelativePureForGuard(m.raw) {`
# Delta de literal único: `hf.requiresVarOrShellPrefix &&` prefixado com
# `false &&`, tornando o branch morto — Go nunca acusa forma relativa enquanto
# Node.js/Python ficam reais e corretos, expondo a divergência no caso
# cg-claude-relativo (P2 vacuity guard: Go não reporta violação mas deveria).
#
...
sed 's/hf\.requiresVarOrShellPrefix \&\& isRelativePureForGuard(m\.raw)/false \&\& hf.requiresVarOrShellPrefix \&\& isRelativePureForGuard(m.raw)/' \
